...
def get_wholesale_purchase_plan(wholesale_catalog: list[Barrel]):
    ...
    barrelsToBuy = (int)(gold / 100)
    if barrelsToBuy > 10:
        barrelsToBuy = 10

    if numGreenPots < 10:
        for i in range(barrelsToBuy):
            with db.engine.begin() as connection:
                connection.execute(sqlalchemy.text("UPDATE global_inventory SET num_green_ml = num_green_ml + 500"))
            with db.engine.begin() as connection:
                connection.execute(sqlalchemy.text("UPDATE global_inventory SET gold = gold - 100"))
        return [
            {
                "sku": "SMALL_GREEN_BARREL",
                "quantity": barrelsToBuy,
            }
        ]
    else:
        return
